...
function addContent(language) {
  ...
  titles.forEach((title, index) => {
    ...
    button.setAttribute(
      ...
      `ESDC-EDSC:SABAAI:${sanitizeForAnalytics(titlesEN[index])}`
    );
...
function askUserType(message) {
  ...
  employeeButton.onclick = function (e) {
    setUser(1);
    pushbeacon(e, "Learners");
  };

  managerButton.onclick = function (e) {
    setUser(2);
    pushbeacon(e, "Managers");
  };

  adminButton.onclick = function (e) {
    setUser(3);
    pushbeacon(e, "Administrators");
  };

  instructorButton.onclick = function (e) {
    setUser(4);
    pushbeacon(e, "Instructors");
  };

  contentButton.onclick = function (e) {
    setUser(5);
    pushbeacon(e, "Content Administrators");
  };

  contentViewer.onclick = function (e) {
    setUser(6);
    pushbeacon(e, "Content Viewers");
  };
